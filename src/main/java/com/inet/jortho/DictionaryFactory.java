...
/**
 * With the DictionaryFactory you can create / load a Dictionary. A Dictionary is list of words with a API for searching. The list is saved internally as a
 * tree.
 *
 * @see Dictionary
...
class DictionaryFactory {
    ...
    private final static class Node extends ArrayList<NodeEntry> {
        ...
        NodeEntry searchCharOrAdd(final char c) {
            for (int i = 0; i < size(); i++) {
                ...
                add(i, entry);
                trimToSize(); //reduce the memory consumption, there is a very large count of these Nodes.
                return entry;
            }
    ...
    /**
     * Describe a single character in the Dictionary tree.
     */
    ...
    /**
     * Create from the data in this factory a Dictionary object. If there are no word added then the Dictionary is empty. The Dictionary need fewer memory as
    ...
    /**
     * Load the directory from plain a list of words. The words must be delimmited with newlines. This method can be called multiple times.
     *
     * @param stream a InputStream with words
     * @param charsetName the name of a code page for example "UTF8" or "Cp1252"
    ...
    /**
     * Load the directory from plain a list of words. The words must be delimited with newlines. This method can be called multiple times.
     *
    ...
    /**
     * Load the directory from a compressed list of words with UTF8 encoding. The words must be delimited with newlines. This method can be called multiple
     * times.
     *
